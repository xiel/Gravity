...
public class sketch_gravity extends PApplet {
...
int backgroundColor = color(3, 3, 15);
...
float lightLayerDiff = .5f;
float starsFrontLayerDiff = .2f;
float starsBGLayerDiff = .17f;
...
//texts
int textColor = color(100, 255, 255);
...
public void setup(){
	...
	//init stars layer
	starsLayer = createGraphics( PApplet.parseInt(starsLayerSize), PApplet.parseInt(starsLayerSize) );
	starsLayerFront = createGraphics( PApplet.parseInt(starsLayerSize), PApplet.parseInt(starsLayerSize) );
	lightLayer = createGraphics( PApplet.parseInt(starsLayerSize), PApplet.parseInt(starsLayerSize) );
	speedLinesLayer = createGraphics( PApplet.parseInt(starsLayerSize), PApplet.parseInt(starsLayerSize) );
	
	println( PApplet.parseInt(starsLayerSize) );
...
public void calcTimeScore(){
	...
	scoreTimeLvl = round(500 * ((gems.size() * 30 * 1000) / (PApplet.parseFloat(now) - PApplet.parseFloat(lvlStartedAt) )) ) * gems.size();
...
public void updateAndDrawPlayer(){
	...
	if(buttonPressed || nextLvlPending){
		...
		float distanceInfluence = max(0.07f , min(0.12f , 10/distanceToClosestPlanet) );
...
public void initPlanets(int lvl){
...
public void drawGems(){
	for (int i = 0; i < gems.size(); i++) {
		...
		rotate( radians(currentRotation * 1.5f * -1) );
		imageMode(CENTER);
		image(gemImg, 0, 0);
		popMatrix();
	}
...
public void drawFrontStars(){
	starsLayerFront.beginDraw();
	starsLayerFront.noStroke();
	
	for(int i = 0; i < PApplet.parseInt(starsCount / 2); i++){
...
public void serialEvent(Serial port){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "sketch_gravity" };
    if (passedArgs != null) {
